 ...
 *
 * Flags:
 *   --baseline=<path>   Path to the baseline findings JSON (default: .prometheus/findings.json)
...
export function formatDiffConsole(
  ...
  lines.push(`\n${BOLD}Prometheus Diff${RESET}  ${DIM}(baseline: ${baselinePath})${RESET}\n`);
...
export const DEFAULT_BASELINE_FILENAME = '.prometheus/findings.json';
...
export async function cmdDiff(argv: string[]): Promise<void> {
  ...
  if (!baselineExists) {
    process.stdout.write(
      ...
      `Run 'prometheus scan' then 'prometheus diff --save' to create a baseline.\n`,
    );
    process.exit(0);
    return;
  }
  ...
  if (!scan) {
    process.stderr.write(
      'prometheus diff: .prometheus/report.json not found — run prometheus scan first\n',
    );
    process.exit(1);
    return;
  }
