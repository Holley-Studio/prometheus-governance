/**
 * prometheus pack:publish [pack-name] — compile, validate, and npm publish a rule pack
 *
 * Usage:
 *   prometheus pack:publish
 *   prometheus pack:publish my-pack
 *   prometheus pack:publish my-pack --compile --dry-run
 *   prometheus pack:publish my-pack --tag=beta
 *   prometheus pack:publish my-pack --access=restricted
 *
 * Steps:
...
function resolvePack(
  ...
  const localPacksDir = join(root, '.prometheus', 'packs');

  if (!existsSync(localPacksDir)) {
    process.stderr.write(
      'pack:publish: no packs directory found at .prometheus/packs/\n' +
        '  Run: prometheus pack:create @scope/name\n',
    );
    process.exit(1);
  }
  ...
  if (localPacks.length === 0) {
    process.stderr.write(
      'pack:publish: no local packs found in .prometheus/packs/\n' +
        '  Run: prometheus pack:create @scope/name\n',
    );
    process.exit(1);
  }
  ...
  if (packName) {
    ...
    if (!match) {
      process.stderr.write(
        `pack:publish: pack "${packName}" not found in .prometheus/packs/\n` +
  ...
  process.stderr.write(
    '\n  Usage: prometheus pack:publish <pack-name>\n',
  );
  process.exit(1);
...
export async function cmdPackPublish(argv: string[]): Promise<void> {
    ...
    if (compile) {
      ...
      process.stderr.write(
        'pack:publish: warn — provides.rules=true but rules/index.js does not exist.\n' +
          ...
          '  Or re-run with: prometheus pack:publish --compile\n',
      );
  ...
  console.log('');
  console.log('Prometheus discovers packs under node_modules/@prometheus/ automatically.');
}
