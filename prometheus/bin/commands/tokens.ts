/**
 * prometheus tokens:* — AI token cost reporting and budget management.
 *
 * Subcommands:
...
function loadBudgetConfig(root: string): TokenBudgetConfig {
  try {
    const cfgPath = join(root, '.prometheus', 'config.json');
...
export async function cmdTokens(argv: string[]): Promise<void> {
  ...
  switch (sub) {
    ...
    case 'reset': {
      const scope = argv[1];
      if (scope === '--session') {
        const sessionIdFile = join(root, '.prometheus', 'token-session-id');
        ...
        const logFile = join(root, '.prometheus', 'token-usage.jsonl');
        ...
        console.error('Usage: prometheus tokens:reset --session | --all\n');
        process.exitCode = 1;
      }
      break;
    }
    ...
    case 'budget': {
      ...
      console.log('\nPrometheus Token Budget Configuration\n');
      ...
      console.log('');
      console.log('  To enable: add "tokenBudget": { "enabled": true } to .prometheus/config.json\n');
      break;
    }

    default: {
      console.error(
        'Usage: prometheus tokens:<subcommand>\n\n' +
        '  tokens:report          Show usage by session/day/project\n' +
