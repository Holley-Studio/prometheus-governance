/**
 * prometheus commit:lint — validate commit messages against Conventional Commits.
 * prometheus commit:create — interactive guided commit wizard.
 *
 * Usage:
 *   prometheus commit:lint <msg-file>       # Used by commit-msg hook (pass $1)
 *   prometheus commit:lint --last           # Lint the last git commit
 *   prometheus commit:lint --message "..."  # Lint an inline string
 *   prometheus commit:lint --json           # JSON output
 *   prometheus commit:create                # Interactive wizard
 */
...
export async function cmdCommitLint(argv: string[]): Promise<void> {
  ...
  if (!message) {
    process.stderr.write(
      ...
      'Examples:\n' +
      '  prometheus commit:lint "$1"          # from commit-msg hook\n' +
      '  prometheus commit:lint --last        # lint most recent commit\n' +
      '  prometheus commit:lint --message "feat: add login"\n'
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (doCommit) {
      ...
      if (confirm === '' || confirm === 'y' || confirm === 'yes') {
        try {
          const tmpFile = join(root, '.git', 'COMMIT_EDITMSG.prometheus-wizard');
