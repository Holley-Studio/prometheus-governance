...
describe('loadBaselineFindings', () => {
  it('returns [] for a non-existent path', () => {
    const result = loadBaselineFindings('/tmp/__nonexistent_path_prometheus_test__.json');
...
describe('formatDiffConsole', () => {
  ...
  it('shows the baseline path in the header', () => {
    const out = formatDiffConsole(noChangeDiff, '/my/project/.prometheus/findings.json');
    expect(out).toContain('/my/project/.prometheus/findings.json');
  });
...
describe('DEFAULT_BASELINE_FILENAME', () => {
  it('points into .prometheus/', () => {
    expect(DEFAULT_BASELINE_FILENAME).toContain('.prometheus/');
