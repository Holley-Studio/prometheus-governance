/**
 * prometheus scope:* — agent blast radius enforcement.
 *
 * Subcommands:
...
export async function cmdScope(argv: string[]): Promise<void> {
  ...
  switch (sub) {
    case 'init': {
      ...
      console.log(`\nPrometheus Scope Config created: ${scopePath}\n`);
      ...
      console.log('\nEdit .prometheus/scope.json to customize boundaries.\n');
      console.log('Install with: prometheus claude:govern install\n');
      break;
    }

    case 'status': {
      const status = getScopeStatus(root);
      console.log('\nPrometheus Agent Scope\n');
      console.log(`  Config file:   ${status.scopeFilePath}`);
      console.log(`  Configured:    ${status.configured ? '✓ yes' : '✗ no (all operations allowed)'}`);
      if (!status.configured) {
        console.log('\n  Run `prometheus scope:init` to create a scope config.\n');
        return;
      }
    ...
    case 'check': {
      const target = argv[1];
      if (!target) {
        console.error('Usage: prometheus scope:check <file-path|command>\n');
        process.exitCode = 1;
        return;
      }
    ...
    default: {
      console.error(
        'Usage: prometheus scope:<subcommand>\n\n' +
        '  scope:init    Scaffold .prometheus/scope.json with safe defaults\n' +
