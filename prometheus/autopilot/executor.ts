...
function buildTaskPrompt(
  ...
  const parts: string[] = [
    `You are executing task ${taskNum} of ${totalTasks} in a Prometheus Autopilot session for "${plan.project}".`,
    ``,
...
export async function executeSession(
  ...
  for (const task of plan.tasks) {
    ...
    if (task.isCheckpoint) {
      ...
      process.stdout.write(`To continue: prometheus autopilot resume [PLAN_FILE]\n`);
      process.stdout.write(`To abort:    prometheus autopilot cancel\n`);
