...
export function recoverLeftoverProfile(root: string): boolean {
  ...
  process.stderr.write(
    `[prometheus] Detected leftover autopilot permission profile from session ${leftover.sessionId}.\n` +
    `[prometheus] Restoring original settings...\n`
  );

  restorePermissions(root, leftover.backupPath);

  process.stderr.write(`[prometheus] Settings restored.\n`);
  return true;
}
