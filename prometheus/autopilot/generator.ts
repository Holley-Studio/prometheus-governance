 ...
 *
 * Usage:
 *   prometheus autopilot generate "add Stripe checkout to the Express app"
 *   prometheus autopilot generate "add Stripe checkout" --out STRIPE_PLAN.md
 */
import {
  existsSync,
  readFileSync,
...
export function buildClarifyingPrompt(
  ...
  return [
    `You are helping generate a Prometheus Autopilot plan for a software project.`,
    ``,
    `PROJECT CONTEXT:`,
...
export function buildPlanGenerationPrompt(
  ...
  return [
    `You are generating a Prometheus Autopilot MASTER_PLAN.md for a software project.`,
    ``,
    `PROJECT CONTEXT:`,
    `  Name: ${snapshot.projectName}`,
    `  Language: ${snapshot.mainLanguage}`,
...
export function buildGenerationReport(
  ...
  if (errors.length > 0) {
    ...
    lines.push(`    → Run: prometheus autopilot validate ${outputPath}`);
  } else {
  ...
  lines.push(
    ...
    `       prometheus autopilot validate ${outputPath}`,
    ``,
    `  3. When ready, run:`,
    `       prometheus autopilot start ${outputPath}`,
    ``,
    `${'─'.repeat(62)}`,
  );
...
export async function generatePlan(
  ...
  process.stdout.write(`\nPrometheus Autopilot — Plan Generator\n`);
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'prometheus-gen-'));
