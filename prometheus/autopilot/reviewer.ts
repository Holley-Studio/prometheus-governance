 ...
 *
 * Usage:
 *   prometheus autopilot review [session-id]
 *   prometheus autopilot review               ← uses active session
 */
...
export function formatReviewMarkdown(
  ...
  return [
    ...
    `---`,
    ``,
    `*Generated by Prometheus Autopilot reviewer — this is AI-assisted, not a substitute for human review.*`,
  ].join('\n');
}
...
export function displayReview(report: ReviewReport, session: AutopilotSession, reportPath: string): void {
  ...
  process.stdout.write(`  Draft PR: prometheus autopilot open-pr ${session.id}\n`);
  process.stdout.write(`  Discard:  prometheus autopilot revert ${session.id}\n\n`);
}
...
export async function reviewSession(
  ...
  process.stdout.write(`\nPrometheus Autopilot — Session Reviewer\n`);
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'prometheus-review-'));
  ...
  // Write report file
  const reportPath = join(root, `.prometheus/autopilot/${session.id}.review.md`);
