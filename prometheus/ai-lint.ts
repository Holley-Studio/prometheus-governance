/**
 * Prometheus AI Behavior File Linter
 *
 ...
 *
 * Also provides initFromAiConfig() for `prometheus init --from-ai-config`,
 * which reads these files and generates a starter .prometheus/config.json
...
const PROMETHEUS_KEYWORDS = /prometheus|governance|guardrail/i;
...
export function lintAiConfigFiles(
  ...
  const prometheusConfigExists = existsSync(join(root, '.prometheus', 'config.json'));
  ...
  if (files.length === 0) {
    findings.push({
      ...
      suggestion:
        'Run "prometheus adapters" to generate AI behavior files for all your AI tools, ' +
        'or run "prometheus init --from-ai-config" if you have existing config files.',
    });
    return findings;
  }
  ...
  for (const f of files) {
    ...
    if (f.bytes < 200) {
      findings.push({
        ...
        suggestion: 'Add security rules, architecture constraints, and testing guidelines. Run "prometheus adapters" to populate it.',
      });
    ...
    // AICFG_010: .prometheus/config.json exists but AI config doesn't mention Prometheus
    if (prometheusConfigExists && !PROMETHEUS_KEYWORDS.test(f.content)) {
      findings.push({
        severity: 'MEDIUM',
        category: 'ai_config_not_synced',
        ...loc,
        message: `${f.relPath} doesn't mention Prometheus governance, but .prometheus/config.json exists.`,
        suggestion:
          'Run "prometheus adapters" to regenerate AI behavior files with the Prometheus governance rules ' +
...
interface DetectedStack {
  ...
  hasGovGaps: boolean;
  recommendedPreset: 'prometheus/recommended' | 'prometheus/vibe-coding' | 'prometheus/ai-strict';
...
function detectStack(files: AiConfigFile[]): DetectedStack {
  ...
  if (isAiStrict) {
    recommendedPreset = 'prometheus/ai-strict';
  } else if (isAiHeavy || !hasSecurityRules) {
    recommendedPreset = 'prometheus/vibe-coding';
  } else {
    recommendedPreset = 'prometheus/recommended';
  }
...
export function formatAiLintConsole(
  ...
  lines.push(`Prometheus AI-Lint — ${projectName}`);
...
export function formatInitFromAiConfigConsole(result: InitFromAiConfigResult): string {
  ...
  lines.push('Prometheus — init from AI config');
  lines.push(SEP);
  lines.push('');
  ...
  if (result.filesRead.length === 0) {
    ...
    lines.push('       then re-run "prometheus init --from-ai-config".');
    lines.push('');
    lines.push(SEP);
    return lines.join('\n');
  }
  ...
  if (result.configAlreadyExisted) {
    lines.push(`  ℹ️   .prometheus/config.json already exists — skipped.`);
  } else if (result.configWritten) {
    lines.push(`  ✅  Generated ${result.configPath}`);
    lines.push(`       extends: "${stack.recommendedPreset}"`);
  } else {
    lines.push(`  🔍  Dry run — would generate .prometheus/config.json`);
    lines.push(`       extends: "${stack.recommendedPreset}"`);
  }
  ...
  if (result.lintFindings.length > 0) {
    ...
    if (result.lintFindings.length > 5) {
      lines.push(`    … and ${result.lintFindings.length - 5} more (run "prometheus ai-lint" for full report)`);
    }
  }
  ...
  if (!result.configAlreadyExisted && result.configWritten) {
    lines.push('    1. Run "prometheus scan" to generate your baseline report');
    lines.push('    2. Run "prometheus adapters" to sync AI behavior files with governance rules');
    lines.push('    3. Add "prometheus:review" to your CI pipeline');
  } else if (result.configAlreadyExisted) {
    lines.push('    1. Run "prometheus adapters" to sync AI behavior files with governance rules');
    lines.push('    2. Run "prometheus ai-lint" to fix governance gaps in your AI config files');
  }
