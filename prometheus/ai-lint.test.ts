...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'prometheus-ai-lint-'));
});
...
- All code reviewed by Prometheus before merge
- Follow .prometheus/GUARDRAILS.md
`;
...
`.repeat(2));
  ...
  describe('AICFG_010: not synced with Prometheus', () => {
    it('fires when .prometheus/config.json exists but file has no prometheus mention', () => {
      write('.prometheus/config.json', '{}');
      write('CLAUDE.md', `
# Rules
## Security
...
`.repeat(5));
    ...
    it('does not fire when prometheus is mentioned', () => {
      write('.prometheus/config.json', '{}');
      write('CLAUDE.md', GOOD_CLAUDE_MD);
      const files = discoverAiConfigFiles(root);
      const findings = lintAiConfigFiles(root, files);
...
describe('initFromAiConfig', () => {
  ...
  it('detects vibe-coding preset when CLAUDE.md exists', () => {
    ...
    expect(result.stack.recommendedPreset).toBe('prometheus/vibe-coding');
  ...
  it('does not overwrite existing config.json', () => {
    write('CLAUDE.md', GOOD_CLAUDE_MD);
    write('.prometheus/config.json', '{"project":"existing"}');
    const result = initFromAiConfig(root, false);
    expect(result.configWritten).toBe(false);
